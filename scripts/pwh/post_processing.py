...
def main():
    DATA_DIR_syn = "/mnt/DIIR-JK-NAS/data/LungDataPWH/Syn_df_jacky"
    DATA_DIR_vxm = "/mnt/DIIR-JK-NAS/data/LungDataPWH/data_registered/"

    ORIGINAL_IMAGE_DIR_vxm = "/mnt/DIIR-JK-NAS/data/LungDataPWH/data_normalized"
    ...
    for case in cases[0:1]:
        ...
        wrap = sitk.ReadImage(os.path.join(DATA_DIR_syn,case,"{}_ins-{}_exp-1Warp.nii.gz".format(case,case)))
        ...
        generic_affine = sitk.ReadTransform(os.path.join(DATA_DIR_syn,case,"{}_ins-{}_exp-0GenericAffine.mat".format(case,case)))
        # # generic_affine = generic_affine.GetInverse()

        fixed = sitk.ReadImage(os.path.join(ORIGINAL_IMAGE_DIR_vxm,case,"{}_ins.nii".format(case)))
        moving = sitk.ReadImage(os.path.join(ORIGINAL_IMAGE_DIR_vxm,case,"{}_exp.nii".format(case)))
        moved = sitk.ReadImage(os.path.join(DATA_DIR_syn,case,"{}_ins-{}_exp-Warped.nii.gz".format(case,case)))
        ...
        plot_2d_hist(fixed,moving,os.path.join(DATA_DIR_syn,case,"hist_moving.png"))
        plot_2d_hist(fixed,moved,os.path.join(DATA_DIR_syn,case,"hist_moved.png"))
